...
# Replace movimientosOrdenados calculation
old_movimientos_calc = "const movimientosOrdenados = movimientosFiltrados.sort((a, b) => new Date(b.fecha) - new Date(a.fecha));"
new_movimientos_calc = """const getMovimientosProcesados = () => {
    if (!filtroProd) {
...
content = content.replace(old_movimientos_calc, new_movimientos_calc)

# Replace the Table component
old_table_start = '<div className="overflow-x-auto">\n            <table className="w-full text-left">'
old_table_end = '</table>\n          </div>'

# Extract the old table block
table_match = re.search(r'<div className="overflow-x-auto">\s*<table.*?</table>\s*</div>', content, flags=re.DOTALL)
if table_match:
    old_table_block = table_match.group(0)
    
    classic_table = """<div className="overflow-x-auto">
            {filtroProd ? (
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-slate-50 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider">
                    ...
                    <th className="px-3 py-2 border-b text-center bg-indigo-50 text-indigo-700" colSpan="3">EXISTENCIAS (SALDOS)</th>
                  </tr>
...
""" + old_table_block.replace('<div className="overflow-x-auto">', '').replace('</div>', '') + """
            )}
          </div>"""
    content = content.replace(old_table_block, classic_table)
