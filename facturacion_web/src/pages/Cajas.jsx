...
import { Wallet, Plus, Play, Square, RefreshCcw, Printer, History, DollarSign, TrendingUp, X, Pencil, Trash2, Eye, Building2 } from "lucide-react";
...
export default function Cajas() {
  ...
  const [guardandoRemesa, setGuardandoRemesa] = useState(false);
  ...
  const guardarRemesa = async () => {
    if (!remesaForm.monto || parseFloat(remesaForm.monto) <= 0) {
      alert("Ingrese un monto válido mayor a 0");
      return;
    }
    if (!remesaForm.numero_comprobante.trim()) {
      alert("Ingrese el número de comprobante o boleta de depósito");
      return;
    }

    setGuardandoRemesa(true);
    try {
      await api.post(`/api/v1/cajas/sesiones/${sesionActiva.sesion_id}/remesa-banco?empresa_id=${empresaId}&usuario_id=${usuarioId}`, {
        monto: parseFloat(remesaForm.monto),
        banco: remesaForm.banco,
        numero_comprobante: remesaForm.numero_comprobante.trim(),
        notas: remesaForm.notas
      });

      setShowRemesaModal(false);
      setRemesaForm({ monto: "", banco: "Banco Agrícola", numero_comprobante: "", notas: "" });
      cargarSesion();
      window.dispatchEvent(new CustomEvent("avatar:say", { detail: { text: "Remesa bancaria registrada exitosamente." }}));
    } catch (e) {
      alert(e.response?.data?.detail || "Error al registrar la remesa");
    } finally {
      setGuardandoRemesa(false);
    }
  };
  ...
  const formatConcepto = (m) => {
    ...
    if (m.referencia_tipo === "remesa_banco" || text.includes("Remesa")) {
      badge = { label: "REMESA BANCO", color: "bg-blue-100 text-blue-700 border-blue-200" };
    } else if (m.referencia_tipo === "financiamiento" || text.includes("Inyección") || text.includes("Financiamiento")) {
  ...
  return (
    <div className="p-8 max-w-7xl mx-auto pb-24">
              ...
              <div>
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4 border-b border-slate-100 pb-4">
                  ...
                  <div className="flex flex-wrap gap-2">
                    ...
                    <button onClick={() => setShowRemesaModal(true)} className="bg-blue-600 text-white px-4 py-2.5 rounded-xl font-semibold hover:bg-blue-700 flex items-center gap-2 text-sm shadow-md shadow-blue-500/20 transition-all">
                      <Building2 className="w-4 h-4" /> Registrar Remesa
                    </button>
                ...
                <div className="grid grid-cols-2 md:grid-cols-5 gap-3 mb-6">
                  ...
                  <div className="bg-white p-4.5 rounded-2xl border border-slate-200 shadow-sm border-t-4 border-blue-500">
                    <p className="text-xs text-blue-700 uppercase font-extrabold tracking-wider">Remesas al Banco</p>
                    <p className="text-2xl font-black text-blue-700 mt-1">{fmt(sesionActiva.total_remesas || 0)}</p>
                  </div>
          ...
          <div className="bg-white rounded-3xl max-w-3xl w-full p-6 max-h-[90vh] overflow-y-auto shadow-2xl">
            <div className="flex justify-between items-center mb-6 border-b pb-3">
              <div>
                ...
                <p className="text-xs text-slate-500 mt-0.5">Verifique el efectivo físico disponible en la bandeja</p>
              </div>
              <div className="flex items-center gap-2">
                <button
                  type="button"
                  onClick={() => setShowRemesaModal(true)}
                  className="bg-blue-50 border border-blue-200 text-blue-700 hover:bg-blue-100 px-3 py-1.5 rounded-xl text-xs font-bold flex items-center gap-1.5 shadow-xs transition-all"
                >
                  <Building2 className="w-3.5 h-3.5" /> Registrar Remesa al Banco
                </button>
                <button onClick={() => setShowArqueoModal(false)} className="text-slate-400 hover:text-slate-600 text-lg px-2">&times;</button>
              </div>
            </div>
            ...
            <div className="bg-slate-50 p-4 rounded-2xl border border-slate-200 mb-4">
              ...
              <div className="flex justify-between items-center mb-2">
                <div className="flex items-center gap-2">
                  ...
                  {(sesionActiva.total_remesas > 0) && (
                    <span className="text-[10px] font-bold bg-blue-100 text-blue-700 border border-blue-200 px-2 py-0.5 rounded-md">
                      Descontado Remesas ({fmt(sesionActiva.total_remesas)})
                    </span>
                  )}
                </div>
      ...
      {/* Modal Registrar Remesa al Banco */}
      {showRemesaModal && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl p-6 w-full max-w-lg shadow-2xl border border-slate-100 animate-in fade-in zoom-in duration-200">
            <div className="flex justify-between items-center pb-4 border-b border-slate-100 mb-5">
              <h2 className="text-lg font-bold text-slate-800 flex items-center gap-2">
                <Building2 className="w-5 h-5 text-blue-600" /> Registrar Remesa al Banco
              </h2>
              <button onClick={() => setShowRemesaModal(false)} className="text-slate-400 hover:text-slate-600">
                <X className="w-5 h-5" />
              </button>
            </div>

            <div className="bg-blue-50/70 border border-blue-200 rounded-2xl p-3.5 mb-4 text-xs text-blue-900 leading-relaxed">
              💡 <strong>Impacto en Arqueo:</strong> Al registrar la remesa se registrará un egreso de efectivo por traslado al banco, reduciendo automáticamente el <strong>Efectivo Esperado en Caja</strong>.
            </div>

            <div className="space-y-4">
              <div>
                <label className="block text-xs font-semibold text-slate-500 uppercase mb-1">Banco Destino *</label>
                <select 
                  value={remesaForm.banco} 
                  onChange={e => setRemesaForm({ ...remesaForm, banco: e.target.value })}
                ...
                >
                  ...
                  <option value="Otro Banco">Otro Banco / Caja Social</option>
                </select>
              </div>

              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-semibold text-slate-500 uppercase mb-1">Monto Depositado ($) *</label>
                  <div className="relative">
                    <span className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 font-medium">$</span>
                    <input 
                      type="number" 
                      ...
                      min="0.01"
                      placeholder="0.00"
                      value={remesaForm.monto}
                      onChange={e => setRemesaForm({ ...remesaForm, monto: e.target.value })}
                      className="w-full pl-7 pr-3 py-2 bg-slate-50 border border-slate-200 rounded-xl text-sm font-bold focus:outline-none focus:ring-2 focus:ring-blue-500"
                    />
                  </div>
                </div>

                <div>
                  <label className="block text-xs font-semibold text-slate-500 uppercase mb-1">N° Comprobante / Boleta *</label>
                  <input 
                    type="text" 
                    placeholder="Ej: 1049284"
                    value={remesaForm.numero_comprobante}
                    onChange={e => setRemesaForm({ ...remesaForm, numero_comprobante: e.target.value })}
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-500 uppercase mb-1">Observaciones / Notas</label>
                <textarea 
                  rows={2}
                  placeholder="Detalles adicionales del envío o transportadora..."
                  value={remesaForm.notas}
                  onChange={e => setRemesaForm({ ...remesaForm, notas: e.target.value })}
            ...
            <div className="flex gap-3 mt-6">
              <button 
                onClick={() => setShowRemesaModal(false)}
                className="flex-1 px-4 py-2.5 border border-slate-200 text-slate-600 rounded-xl font-medium text-sm hover:bg-slate-50"
              >
                Cancelar
              </button>
              <button 
                onClick={guardarRemesa}
                disabled={guardandoRemesa || !remesaForm.monto || !remesaForm.numero_comprobante.trim()}
                className="flex-1 px-4 py-2.5 bg-blue-600 text-white rounded-xl font-medium text-sm hover:bg-blue-700 disabled:opacity-50 shadow-md shadow-blue-500/20"
              >
                {guardandoRemesa ? "Registrando..." : "Registrar Remesa"}
              </button>
            </div>
          </div>
        </div>
