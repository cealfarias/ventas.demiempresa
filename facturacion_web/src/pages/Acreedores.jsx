    ...
    <div className="p-8 max-w-7xl mx-auto pb-24">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
        <div>
          <h1 className="text-2xl font-bold text-slate-800 flex items-center gap-2">
            <CreditCard className="w-6 h-6 text-indigo-600" /> Directorio Maestro de Acreedores
          </h1>
          <p className="text-sm text-slate-500 mt-1">Catálogo centralizado de entidades financieras, bancos e inversionistas</p>
        </div>
        <div className="flex items-center gap-3">
          <Link
            to="/pago-prestamos"
            className="bg-emerald-600 hover:bg-emerald-700 text-white px-4 py-2.5 rounded-xl text-sm font-bold flex items-center gap-2 shadow-md shadow-emerald-600/20 transition-all"
          >
            <Calculator className="w-4 h-4" /> Préstamos y Amortizaciones
          </Link>
          <button
            onClick={abrirNuevoAcreedor}
            className="bg-indigo-600 hover:bg-indigo-700 text-white px-5 py-2.5 rounded-xl text-sm font-semibold flex items-center gap-2 shadow-md shadow-indigo-600/20 transition-all"
          >
      ...
      {cargando ? (
        ...
        <div className="text-center py-20 text-slate-400 bg-white border border-slate-200 rounded-2xl">
          ...
          <p className="text-sm text-slate-400 mt-1">Agregue un nuevo acreedor para gestionar sus contratos de préstamos.</p>
        </div>
      ) : (
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-slate-50 border-b text-xs uppercase text-slate-500 font-semibold">
                <th className="px-5 py-3.5">Acreedor / Prestamista</th>
                <th className="px-5 py-3.5 text-center">Tasa Referencial</th>
                <th className="px-5 py-3.5 text-right">Saldo Deuda Capital</th>
              ...
              {acreedoresFiltrados.map(a => (
                <tr key={a.id} className="hover:bg-slate-50 transition-colors">
                  ...
                  <td className="px-5 py-4 text-right flex justify-end gap-2 items-center">
                    <button
                      onClick={() => navigate(`/pago-prestamos?acreedor_id=${a.id}`)}
                      className="text-xs bg-indigo-50 text-indigo-700 px-3 py-1.5 rounded-lg font-bold border border-indigo-200 hover:bg-indigo-100 flex items-center gap-1 shadow-sm"
                    >
                      <ChevronRight className="w-3.5 h-3.5" /> Ver Préstamos
                    </button>
                    <button
                      onClick={() => abrirHistorial(a)}
                      className="text-xs bg-slate-100 text-slate-700 px-2.5 py-1.5 rounded-lg hover:bg-slate-200 font-medium flex items-center gap-1"
                    >
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md p-6 border border-slate-100 max-h-[90vh] overflow-y-auto">
            ...
            <div className="space-y-3.5">
              <div>
                ...
                <input type="text" value={formAcreedor.nombre} onChange={e => setFormAcreedor({...formAcreedor, nombre: e.target.value})} className="w-full mt-1 px-3 py-2 border rounded-xl text-slate-700" placeholder="Ej: Banco Agrícola / Carlos Mendoza" />
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-xs font-semibold text-slate-500 uppercase">Tasa Interés Referencial (%)</label>
                ...
                {!acreedorEditando && (
                  <div>
                    <label className="text-xs font-semibold text-slate-500 uppercase">Deuda Inicial ($)</label>
              ...
              <div>
                ...
                <textarea rows="2" value={formAcreedor.notas} onChange={e => setFormAcreedor({...formAcreedor, notas: e.target.value})} className="w-full mt-1 px-3 py-2 border rounded-xl text-sm" placeholder="Condiciones generales, representante legal, etc." />
              </div>
            </div>
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-4xl p-6 max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-start mb-4 border-b pb-3">
              <div>
                <h2 className="text-lg font-bold text-slate-800">Historial Financiero del Acreedor</h2>
