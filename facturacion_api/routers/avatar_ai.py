...
MODULOS_KNOWLEDGE = {
    ...
    "/configuracion-contable": {
        ...
        "guia": "1. **Conexión Automática Activa:** La vinculación entre Facturación y Contabilidad se sincroniza automáticamente en el ecosistema (demiempresa.online). No requiere ingresar URLs ni API Keys manuales.\n2. **Mapeo Humanizado de Cuentas:** Mapea de forma clara el nombre humanizado y código de tu catálogo para cada operación (Caja General, Bancos, Débito IVA, Crédito IVA, CxC, CxP, Ventas CF, Ventas CCF, Inventarios, Costo de Ventas).\n3. **Explorador de Árbol y Búsqueda Inteligente:** Haz clic en '🌳 Árbol / Buscar' para realizar búsquedas en tiempo real o explorar tu catálogo jerárquico nivel por nivel.\n4. **Transmisión de Ventas:** Las facturas CCF y Crédito se transmiten automáticamente en tiempo real. Las ventas a Consumidor Final se envían mediante el Resumen Diario Contable.",
        "faqs": "P: ¿Qué hace si el mes está cerrado?\nR: Contabilidad es la autoridad técnica; si un mes se cierra, devolverá un rechazo 400 controlado protegiendo la integridad del período fiscal."
    },
...
PROCEDIMIENTOS_KNOWLEDGE = [
    ...
    },
    {
        ...
        "keywords": ["configurar integracion", "configurar integración", "configurar contabilidad", "integracion contable", "integración contable", "como conecto contabilidad", "cómo conecto contabilidad", "como configurar", "cómo configuro", "pasos integracion", "pasos integración", "api key contable", "mapeo de cuentas", "conectar contabilidad", "configuracion contable", "configuración contable", "arbol contable", "árbol contable", "busqueda inteligente cuentas"],
        "respuesta": "📐 **Paso a Paso para la Integración Contable del Ecosistema:**\n\n1️⃣ **Conexión Automática Activa:**\n   • La integración se sincroniza automáticamente en el ecosistema `demiempresa.online` (sin requerir digitar URLs ni API Keys manuales).\n\n2️⃣ **Mapeo Humanizado de Cuentas (`/configuracion-contable`):**\n   • Ve a **Configuración -> Integración Contable**.\n   • Visualiza y asigna las cuentas de tu catálogo para cada concepto (Caja, Bancos, IVA, Ventas CF, CCF, Inventario, Costo).\n\n3️⃣ **Explorador de Árbol Jerárquico y Búsqueda Inteligente:**\n   • Presiona el botón **'🌳 Árbol / Buscar'** en cualquier casilla para buscar por nombre/código en tiempo real o explorar tu catálogo ordenado por niveles.\n\n4️⃣ **Transmisión de Partidas:**\n   • **Ventas CCF y Crédito:** Se transmiten **automáticamente en tiempo real** al emitir la factura.\n   • **Ventas Consumidor Final:** Se consolidan mediante el módulo **Resumen Diario Contable** (`/resumen-diario-contable`).\n   • **Validación Contable:** Contabilidad audita el Principio de Partida Doble (Debe == Haber) y verifica que el período mensual no esté cerrado.",
